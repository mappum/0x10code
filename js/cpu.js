...
CPU.prototype = {
    ...
    skipUntilNonif: function() {
    	...
    	var opcode;
        do {
          var opcode = this.mem[this.mem.pc] & 0x1f;
          this.skip();
    ...
    step: function() {
    	if(!this.paused) {
	        ...
	        switch (insn.opcode) {
	            ...
	            // MDI
	            case 0x9:
	            	this.set(insn.b, (aVal === 0) ? 0x0000 : getSigned(bVal) % getSigned(aVal));
	            	break;
	            ...
	            case 0x240:
	            	if(this._devices[aVal] && this._devices[aVal].onInterrupt) {
	            		this.paused = true;
	            		var _this = this;
	            		this._devices[aVal].onInterrupt(function(){ _this.paused = false; });
	            	}
	            	break;
	        }
        }
